...
const ChatScreen: React.FC<ChatScreenProps> = ({ config, onExit }) => {
  ...
  const [selectedCallTargets, setSelectedCallTargets] = useState<string[]>([]);
  const [tempSelectedTargets, setTempSelectedTargets] = useState<string[]>([]);
  ...
  useEffect(() => {
     if (!config.roomKey || !isRoomReady || !user) return;

     const callRef = doc(db, 'chats', config.roomKey, 'calls', 'active_call');
     
     const unsubscribe = onSnapshot(callRef, (snapshot) => {
         if (snapshot.exists()) {
             const data = snapshot.data();
             
             // Check if this call is meant for us
             // If targetUids is set, and we are not in it (and we are not the creator), ignore it.
             if (data.targetUids && 
                 Array.isArray(data.targetUids) && 
                 data.targetUids.length > 0 && 
                 !data.targetUids.includes(user.uid) && 
             ...
             setActiveCall(false);
             if (showCallModal) {
                 setShowCallModal(false); // Auto close if call deleted
             }
         }
     }, (err) => console.log("Call listener error", err));

     return () => unsubscribe();
  }, [config.roomKey, isRoomReady, showCallModal, user]);
  ...
  useEffect(() => {
     ...
     const unsubscribe = onSnapshot(q, (snapshot: QuerySnapshot<DocumentData>) => {
         ...
         setActiveUsersList(currentUsers);
  ...
  const handleDeleteChat = async () => {
    ...
    try {
        ...
        await Promise.allSettled([
            ...
            deleteCollection("calls") // Cleanup calls collection too
        ]);
  ...
  const handleStartCallClick = (type: 'audio' | 'video') => {
      ...
      const otherUsers = activeUsersList.filter(u => u.uid !== user?.uid);
      if (otherUsers.length === 0) {
          // No one else is online, just start call directly (waiting mode)
          setSelectedCallTargets([]);
          setIsHost(true);
          setShowCallModal(true);
      } else {
          // Open selection modal
          setTempSelectedTargets([]); 
          setShowUserSelectModal(true);
      }
  };

  const confirmStartCall = () => {
      setSelectedCallTargets(tempSelectedTargets);
      setIsHost(true);
      setShowUserSelectModal(false);
      setShowCallModal(true);
  };

  const toggleTargetSelection = (uid: string) => {
  ...
  const toggleSelectAll = () => {
      const otherUsers = activeUsersList.filter(u => u.uid !== user?.uid);
      if (tempSelectedTargets.length === otherUsers.length) {
          setTempSelectedTargets([]);
      } else {
          setTempSelectedTargets(otherUsers.map(u => u.uid));
      }
  };
    ...
    <div className="fixed inset-0 flex flex-col h-[100dvh] w-full bg-slate-100 dark:bg-slate-900 max-w-5xl mx-auto shadow-2xl overflow-hidden z-50 md:relative md:inset-auto md:rounded-2xl md:my-4 md:h-[95vh] md:border border-white/40 dark:border-slate-800 transition-colors">
      ...
      {showCallModal && user && (
          <CallModal 
            ...
            isHost={isHost}
            targetUids={selectedCallTargets}
      ...
      <header className="glass-panel px-3 py-3 flex items-center justify-between z-10 sticky top-0 shadow-sm pt-[calc(0.75rem+env(safe-area-inset-top))]">
        ...
        <div className="flex gap-1 sm:gap-2 flex-shrink-0 items-center">
            ...
            {activeCall && !showCallModal && (
                <button 
                    onClick={() => { setIsHost(false); setShowCallModal(true); }}
                    className="p-2 rounded-lg transition-all animate-in zoom-in duration-300 bg-green-500 text-white shadow-lg shadow-green-500/30 animate-pulse flex items-center gap-1"
                    title="Join Active Call"
                >
                    {callType === 'audio' ? <Phone size={20} fill="currentColor" /> : <Video size={20} fill="currentColor" />}
                    <span className="text-xs font-bold hidden sm:inline">Join</span>
                </button>
            )}

            {/* Start Call Buttons (only if no active call) */}
            {!activeCall && !showCallModal && (
                <>
                    <button 
                        onClick={() => handleStartCallClick('audio')}
                        className="p-2 rounded-lg transition-all animate-in zoom-in duration-300 text-slate-400 hover:text-green-500 hover:bg-green-50 dark:hover:bg-green-900/20"
      ...
      <footer className="bg-white dark:bg-slate-900 p-1.5 border-t border-slate-200 dark:border-slate-800 shadow-lg z-20 relative pb-[calc(0.75rem+env(safe-area-inset-bottom))] flex flex-col items-center justify-center transition-colors">
         {/* Footer elements (inputs, etc) remain same as before... */}
      ...
      {/* User Selection Modal for Calls */}
      {showUserSelectModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
              <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-sm w-full shadow-2xl scale-100 animate-in zoom-in-95 duration-200 border border-white/10 dark:border-slate-800">
                  <div className="flex flex-col gap-4">
                      <div className="flex items-center justify-between">
                        <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                             {callType === 'audio' ? <Phone size={20} className="text-blue-500"/> : <Video size={20} className="text-blue-500"/>}
                             Start {callType === 'audio' ? 'Audio' : 'Video'} Call
                        </h3>
                        <button onClick={() => setShowUserSelectModal(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                            <X size={20} />
                        </button>
                      </div>
                      
                      <p className="text-sm text-slate-500 dark:text-slate-400">
                          Select who to call. If you select none, anyone can join.
                      </p>

                      <div className="max-h-60 overflow-y-auto space-y-2 py-2">
                          {activeUsersList.filter(u => u.uid !== user?.uid).length === 0 ? (
                               <p className="text-center text-sm text-slate-400 py-4">No other users online.</p>
                          ) : (
                              activeUsersList.filter(u => u.uid !== user?.uid).map(u => {
                                  const isSelected = tempSelectedTargets.includes(u.uid);
                                  return (
                                      <button 
                                        key={u.uid}
                                        onClick={() => toggleTargetSelection(u.uid)}
                                        className={`w-full flex items-center p-3 rounded-xl border transition-all ${isSelected 
                                            ? 'bg-blue-50 border-blue-200 dark:bg-blue-900/20 dark:border-blue-800' 
                                            : 'bg-slate-50 border-slate-200 dark:bg-slate-800 dark:border-slate-700'}`}
                                      >
                                          <img src={u.avatar} alt={u.username} className="w-8 h-8 rounded-full mr-3 bg-slate-200" />
                                          <span className="flex-1 text-left font-medium text-slate-800 dark:text-slate-100">{u.username}</span>
                                          {isSelected 
                      ...
                      <div className="flex justify-between items-center pt-2">
                           <button onClick={toggleSelectAll} className="text-xs font-semibold text-blue-500 hover:underline">
                               {tempSelectedTargets.length === activeUsersList.filter(u => u.uid !== user?.uid).length ? 'Deselect All' : 'Select All'}
                           </button>
                           <span className="text-xs text-slate-400">
                               {tempSelectedTargets.length} selected
                           </span>
                      </div>

                      <button 
                          onClick={confirmStartCall}
                          className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg shadow-blue-500/30 transition transform active:scale-95"
                      >
                          Start Call
                      </button>
                  </div>
