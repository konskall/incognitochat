...
const MessageItem = React.memo(({ msg, isMe, currentUid, roomOwnerUid, onEdit, onRequestDelete, onReact, onReply, onRetry, onPreview, onUserClick, onIncoClick, getSeenBy, displayAvatar, searchQuery, showSeen, isOwner, isPinned, onPin, onUnpin, onVotePoll, onToggleClosedPoll, isFirstOfGroup = true, isLastOfGroup = true }: {
  ...
  const onTouchEnd = () => {
    if (Math.abs(swipeX) > 55 && !msg.status) onReply(msg); // no reply to a not-yet-persisted bubble (dead temp id)
    setSwipeX(0);
    ...
    <div id={`msg-${msg.id}`} style={CV_STYLE} className={`relative flex w-full ${isLastOfGroup ? 'mb-3' : 'mb-0.5'} animate-in slide-in-from-bottom-2 duration-300 group ${isMe ? 'justify-end' : 'justify-start'}`}>
      ...
      >
        ...
        {!isMe && (
          isFirstOfGroup ? (
            <img
              ...
              onClick={() => isBot ? onIncoClick?.(displayAvatar) : onUserClick?.(msg.uid, msg.username, displayAvatar)}
              role="button"
              tabIndex={0}
              aria-label={isBot ? 'About inco' : `View ${msg.username}'s profile`}
              onKeyDown={onActivate(() => isBot ? onIncoClick?.(displayAvatar) : onUserClick?.(msg.uid, msg.username, displayAvatar))}
              className="w-8 h-8 rounded-full shadow-sm object-cover border-2 border-white dark:border-slate-700 select-none bg-slate-200 dark:bg-slate-700 self-start focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 cursor-pointer hover:scale-110 transition-transform active:scale-95"
            />
          ) : (
            <div className="w-8 shrink-0" aria-hidden="true" />
          )
        )}
        {/* Bubble + reactions stack vertically, aligned to the sender's side, so
            reactions sit directly under the bubble instead of beside it — beside
            placement shoved them far off for wide bubbles. min-w-0 lets the bubble
            still grow to the wrapper's max-width. */}
        ...
        <div
          ...
          tabIndex={0}
          aria-haspopup={msg.status ? undefined : 'menu'}
          aria-keyshortcuts={msg.status ? undefined : 'Enter Space'}
          onKeyDown={(e) => {
          ...
          className={`chat-bubble relative px-4 py-2.5 rounded-2xl text-sm md:text-base min-w-0 max-w-full transition-all select-none cursor-default border shadow-[0_4px_14px_-4px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.18)] dark:shadow-[0_6px_16px_-5px_rgba(0,0,0,0.7),inset_0_1px_0_rgba(255,255,255,0.08)] focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-1 ${isMe ? 'bg-blue-600/90 text-white border-white/25 rounded-br-none' : isBot ? 'bg-indigo-100/90 dark:bg-indigo-800/90 text-indigo-900 dark:text-white border-indigo-300/50 dark:border-indigo-400/30 rounded-bl-none' : 'bg-white/90 dark:bg-slate-900/85 text-slate-800 dark:text-slate-50 border-black/5 dark:border-white/12 rounded-bl-none'} ${isMe && msg.status === 'sending' ? 'opacity-70' : ''}`}>
                {!isMe && isFirstOfGroup && <p className={`text-[10px] font-bold ${isBot ? 'text-indigo-600 dark:text-indigo-200' : 'text-slate-400'} mb-0.5 tracking-wide select-none flex items-center gap-1 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 rounded cursor-pointer transition-colors ${isBot ? 'hover:text-indigo-500' : 'hover:text-blue-500'}`} onClick={() => isBot ? onIncoClick?.(displayAvatar) : onUserClick?.(msg.uid, msg.username, displayAvatar)} role="button" tabIndex={0} aria-label={isBot ? 'About inco' : `View ${msg.username}'s profile`} onKeyDown={onActivate(() => isBot ? onIncoClick?.(displayAvatar) : onUserClick?.(msg.uid, msg.username, displayAvatar))}>{msg.username} {isBot && <Wand2 size={10} className="text-indigo-500 dark:text-indigo-300 animate-pulse" />}</p>}
            ...
            {msg.reactions && Object.keys(msg.reactions).length > 0 && <div className={`flex flex-wrap gap-1 mt-1 ${isMe ? 'justify-end' : 'justify-start'}`}>{Object.entries(msg.reactions).map(([emoji, uids]) => { if (uids.length === 0) return null; const iReacted = uids.includes(currentUid); return (<button key={emoji} onClick={() => onReact(msg, emoji)} aria-pressed={iReacted} aria-label={`React with ${emoji}, ${uids.length} ${uids.length === 1 ? 'reaction' : 'reactions'}${iReacted ? ', you reacted' : ''}`} className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs shadow-sm border transition-all hover:scale-105 animate-in zoom-in-95 duration-200 ${iReacted ? 'bg-blue-100 dark:bg-blue-900/30 border-blue-200 dark:border-blue-800 text-slate-800 dark:text-blue-100' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300'}`}><Emoji emoji={emoji} size={18} /><span className={`font-semibold text-[11px] ${iReacted ? 'text-blue-600 dark:text-blue-300' : 'text-slate-500 dark:text-slate-400'}`}>{uids.length}</span></button>);})}</div>}
        </div>
        </div>
        {menu && (
          <MessageActionMenu
