    ...
    <div id={`msg-${msg.id}`} style={CV_STYLE} className={`relative flex w-full ${isLastOfGroup ? 'mb-3' : 'mb-0.5'} animate-in slide-in-from-bottom-2 duration-300 group ${isMe ? 'justify-end' : 'justify-start'}`}>
      ...
      >
        ...
        <div
          ...
          className={`chat-bubble relative px-4 py-2.5 rounded-2xl text-sm md:text-base min-w-0 transition-all select-none cursor-default border backdrop-blur-xl backdrop-saturate-150 shadow-[0_4px_14px_-4px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.18)] focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-1 ${isMe ? 'bg-blue-600/80 text-white border-white/30 rounded-br-none' : isBot ? 'bg-indigo-100/80 dark:bg-indigo-800/80 text-indigo-900 dark:text-white border-indigo-300/50 dark:border-indigo-300/40 rounded-bl-none' : 'bg-white/80 dark:bg-slate-900/75 text-slate-800 dark:text-slate-50 border-black/5 dark:border-white/15 rounded-bl-none'}`}>
